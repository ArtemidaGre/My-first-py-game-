...
if history==1:
    ...
    itp1=itp2=itp3=0
    magazine('широкий меч', 'рыцарский щит', 'лук с огорода', 50, 34, 90)
    if itp1==1:
        damage=damage+7
    if itp2==1:
        hp=hp+8
...
if choise=="да":
    ...
    magazine('Таблетки от спида', 'Таблетки от головы', 'Таблетки от таблеток', 5, 15, 30)
    if itp1==1:
        print('у вас больше не будет СПИДа!')
        Spid=0
    if itp2==1:
...
magazine('широкий меч', 'броня из адамантия(так сказал продавец)', 'легендарный меч Путина', 80, 110, 900)
if itp1==1:
    damage=damage+13
if itp2==1:
    hp=hp+60
